...
function renderNav(){
  ...
  const sypBtn=document.getElementById('mode-syp');
  const spfBtn=document.getElementById('mode-spf');
  const logo=document.getElementById('app-logo');
  if(sypBtn&&spfBtn){
    sypBtn.style.background=S.tradingMode==='syp'?'var(--accent)':'transparent';
    sypBtn.style.color=S.tradingMode==='syp'?'var(--bg)':'var(--muted)';
    spfBtn.style.background=S.tradingMode==='spf'?'var(--accent)':'transparent';
    spfBtn.style.color=S.tradingMode==='spf'?'var(--bg)':'var(--muted)';
  }
  if(logo)logo.textContent=S.tradingMode==='spf'?'SPF ANALYTICS':'SYP ANALYTICS';
}
...
function renderBreadcrumbs(){
  ...
  const group=groups.find(g=>g.items.includes(S.view));
...
function render(){
  ...
  // Handle SPF mode views
  if(S.tradingMode==='spf'){
    const navItem=getNav().find(n=>n.id===S.view);
    document.getElementById('title').textContent=(navItem?.icon||'')+' '+(navItem?.label||'');
    if(typeof renderSPFView==='function'&&renderSPFView()){
      return;
    }
  }
