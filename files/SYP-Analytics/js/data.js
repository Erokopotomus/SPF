...
async function saveAllLocal(){
  ...
  // SPF data
  await dbSet('reloadInventory',S.reloadInventory);
  await dbSet('transitInventory',S.transitInventory);
  await dbSet('activeOffers',S.activeOffers);
  await dbSet('outboundTrucks',S.outboundTrucks);
  await dbSet('offerHistory',S.offerHistory);
  ...
  // SPF data (localStorage)
  SS('reloadInventory',S.reloadInventory);
  SS('transitInventory',S.transitInventory);
  SS('activeOffers',S.activeOffers);
  SS('outboundTrucks',S.outboundTrucks);
  SS('offerHistory',S.offerHistory);
  SS('reloadFilter',S.reloadFilter);
  SS('transitFilter',S.transitFilter);
...
async function loadAllLocal(){
  ...
  // SPF data
  S.reloadInventory=await dbGet('reloadInventory',LS('reloadInventory',[]));
  S.transitInventory=await dbGet('transitInventory',LS('transitInventory',[]));
  S.activeOffers=await dbGet('activeOffers',LS('activeOffers',[]));
  S.outboundTrucks=await dbGet('outboundTrucks',LS('outboundTrucks',[]));
  S.offerHistory=await dbGet('offerHistory',LS('offerHistory',[]));
}
